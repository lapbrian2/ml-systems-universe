...
export const sections: ChapterSection[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        latex: '\\text{Pipeline Bubble} = \\frac{ {{stages}} - 1}{ {{microBatches}} } = {{bubblePct}}\\%, \\quad \\text{Efficiency} = {{efficiency}}\\%',
        variables: [
          { name: 'stages', label: 'Pipeline stages', min: 2, max: 16, step: 1, default: 4, unit: 'stages' },
          { name: 'microBatches', label: 'Micro-batches', min: 2, max: 64, step: 1, default: 16, unit: '' },
        ],
        computeResult: '(1 - (stages - 1) / microBatches) * 100',
        resultLabel: 'Pipeline efficiency (%)',
      },
      {
        type: 'inline-check',
        question: 'In hybrid (3D) parallelism for training very large models, which parallelism strategy is typically used within a single node?',
        options: [
          'Data parallelism — because nodes have the most storage',
          'Pipeline parallelism — because layer splitting requires minimal bandwidth',
          'Tensor parallelism — because intra-node NVLink provides the highest bandwidth needed for layer splitting',
          'No parallelism is used within a single node',
        ],
        correctIndex: 2,
        explanation: 'Tensor parallelism requires high-bandwidth communication between devices because partial results must be combined at every layer. NVLink within a node provides 600+ GB/s, making it ideal for tensor parallelism. Pipeline parallelism has more modest bandwidth requirements and is used across nodes. Data parallelism is used across groups of nodes.',
      },
    ],
    order: 2,
    keyConcepts: [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'inline-check',
        question: 'Why does BFloat16 (BF16) not require loss scaling, while FP16 does?',
        options: [
          'BF16 has more mantissa bits than FP16',
          'BF16 has the same exponent range as FP32, so small gradient values do not underflow to zero',
          'BF16 uses a different rounding strategy',
          'BF16 automatically applies loss scaling internally',
        ],
        correctIndex: 1,
        explanation: 'FP16 has only 5 exponent bits, limiting its range — values smaller than ~6e-8 underflow to zero, which loses small gradient values. BF16 has 8 exponent bits (same as FP32), so it can represent the same range of values including very small gradients. The trade-off is that BF16 has less precision (7 mantissa bits vs FP16\'s 10), but this matters less for training.',
      },
      {
        type: 'stat',
        value: 2,
        suffix: 'x',
        label: 'training throughput improvement from mixed precision on GPUs with tensor cores — essentially free performance',
      },
    ],
    order: 3,
    keyConcepts: [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'aha',
        highlight: 'For very large models, gradient communication can take longer than the computation itself.',
        explanation: 'A 7 billion parameter model produces 28 GB of FP32 gradients per synchronization step. On a 100 Gbps InfiniBand link, transferring this takes ~2.2 seconds — which can exceed the actual forward and backward pass computation time. At 175B parameters, it is 25x worse. This is why gradient compression, communication-computation overlap, and topology-aware parallelism are not optimizations — they are requirements for training at scale.',
        analogy: 'Imagine a factory assembly line where the workers (GPUs) are incredibly fast, but the conveyor belt (network) between them moves slowly. Adding more workers does not help if the conveyor belt is the bottleneck. You either need a faster belt (better network), smaller packages (gradient compression), or a smarter workflow that keeps workers busy while packages are in transit (communication overlap).',
      },
      {
        type: 'playground',
        title: 'Communication Overhead Calculator',
        description: 'Estimate the gradient synchronization time for your distributed training setup.',
        parameters: [
          { name: 'paramsB', label: 'Model parameters', min: 0.1, max: 175, step: 0.1, default: 7, unit: 'B' },
          { name: 'bandwidthGbps', label: 'Network bandwidth', min: 10, max: 800, step: 10, default: 100, unit: 'Gbps' },
          { name: 'compression', label: 'Compression ratio', min: 1, max: 100, step: 1, default: 1, unit: 'x' },
        ],
        computeFn: 'communicationOverhead',
        chartType: 'gauge',
      },
    ],
    order: 4,
