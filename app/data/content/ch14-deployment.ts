...
export const sections: ChapterSection[] = [
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        value: 14,
        suffix: ' KB',
        label: 'model size for an always-on keyword detection neural network — small enough to fit in MCU flash memory',
      },
      {
        type: 'playground',
        title: 'MCU Memory Budget Calculator',
        description: 'Estimate whether your model fits within an MCU\'s memory constraints. Model weights go in flash, activations go in SRAM.',
        parameters: [
          { name: 'modelParams', label: 'Model parameters (K)', min: 1, max: 500, step: 1, default: 50, unit: 'K' },
          { name: 'bitsPerWeight', label: 'Bits per weight', min: 1, max: 32, step: 1, default: 8, unit: 'bits' },
          { name: 'flashKB', label: 'MCU flash memory', min: 64, max: 4096, step: 64, default: 1024, unit: 'KB' },
          { name: 'sramKB', label: 'MCU SRAM', min: 16, max: 512, step: 16, default: 256, unit: 'KB' },
        ],
        computeFn: 'mcuMemoryBudget',
        chartType: 'gauge',
      },
      {
        type: 'inline-check',
        question: 'Why must intermediate activations reside in SRAM rather than flash memory during MCU inference?',
        options: [
          'Flash memory is too expensive for temporary storage',
          'SRAM supports fast random read/write needed for constantly changing activation values, while flash is read-only or has very slow write speeds',
          'MCU operating systems require flash for the kernel',
          'Flash memory is always smaller than SRAM on MCUs',
        ],
        correctIndex: 1,
        explanation: 'Flash memory is designed for read-mostly storage — writing to flash is slow, requires erasing entire blocks, and wears out the memory over time. Model weights are read-only during inference, so flash is perfect for them. But intermediate activations change with every inference step and need fast random read/write access, which only SRAM can provide.',
      },
    ],
    order: 0,
    keyConcepts: [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'aha',
        highlight: 'An efficient architecture at full precision often beats a large architecture compressed to the same size.',
        explanation: 'This is one of the most underappreciated facts in ML deployment. Engineers instinctively reach for a familiar large model (ResNet-152, BERT-large) and then spend weeks compressing it to fit on-device. But a model designed for efficiency from the start (MobileNetV3, TinyBERT) already uses parameter-efficient operations and can achieve better accuracy at the same size budget. Compression introduces approximation errors; starting small avoids them entirely.',
        analogy: 'It is like packing for a trip: starting with a small, well-organized carry-on bag is almost always better than trying to stuff everything from a large suitcase into a carry-on. The result of cramming is messy, heavy, and you lose things. Starting with the right-sized bag means everything fits naturally.',
      },
      {
        type: 'interactive-equation',
        latex: '\\text{Inference Latency} \\approx \\frac{ {{mflops}} \\text{ MFLOPs}}{{{mhz}} \\text{ MHz} \\times {{efficiency}} } = {{latency}} \\text{ ms}',
        variables: [
          { name: 'mflops', label: 'Model MFLOPs', min: 1, max: 500, step: 1, default: 50, unit: 'MFLOPs' },
          { name: 'mhz', label: 'CPU clock speed', min: 48, max: 400, step: 1, default: 200, unit: 'MHz' },
          { name: 'efficiency', label: 'Utilization factor', min: 0.1, max: 1.0, step: 0.05, default: 0.3, unit: '' },
        ],
        computeResult: 'mflops / (mhz * efficiency) * 1000',
        resultLabel: 'Estimated inference time (ms)',
      },
    ],
    order: 2,
    keyConcepts: [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'inline-check',
        question: 'What makes binary neural networks practical for MCU deployment despite their significant accuracy loss?',
        options: [
          'MCUs have native binary multiply instructions that are extremely fast',
          'Binary networks replace multiply-accumulate with XNOR+popcount bitwise operations, enabling ML on devices that cannot support any higher-precision computation',
          'Binary networks require no training at all',
          'MCUs have unlimited flash storage for binary weights',
        ],
        correctIndex: 1,
        explanation: 'Binary neural networks replace floating-point multiplications with XNOR operations and accumulations with popcount (counting 1-bits), which are simple bitwise CPU instructions available on even the smallest MCUs. While accuracy drops 10-15% on complex tasks, for simple classification tasks the trade-off enables ML on devices with just a few KB of memory — where the alternative is no ML at all.',
      },
      {
        type: 'stat',
        value: 32,
        suffix: 'x',
        label: 'model size reduction from binary (1-bit) quantization vs FP32 — enabling ML on the tiniest devices',
      },
    ],
    order: 4,
    keyConcepts: [
