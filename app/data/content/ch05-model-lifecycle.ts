...
export const sections: ChapterSection[] = [
  {
    ...
    blocks: [
      ...
      {
        type: 'inline-check',
        question: 'What is the most common root cause of ML project failure?',
        options: [
          'Insufficient GPU compute resources',
          'Poor problem formulation and unclear objectives',
          'Using the wrong ML framework',
          'Not having enough training data',
        ],
        correctIndex: 1,
        explanation: 'The majority of failed ML projects fail due to poor problem formulation — building an accurate model for the wrong objective. Before writing code, engineers must clearly define what decision the model will inform, how success will be measured, and what the constraints are.',
      },
      {
        type: 'stat',
        value: 87,
        suffix: '%',
        label: 'of ML projects never make it to production (Gartner), often due to lifecycle management gaps',
      },
      {
        type: 'callout',
        variant: 'note',
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'inline-check',
        question: 'Which of the following should NOT be logged as part of experiment tracking?',
        options: [
        ...
        correctIndex: 2,
        explanation: 'Experiment tracking should log everything needed for reproducibility — hyperparameters, seeds, framework versions, data versions, and hardware specs. However, sensitive credentials like API keys should never be logged, as experiment logs are often shared across teams and stored long-term.',
      },
      {
        type: 'stat',
        value: 200,
        suffix: '+',
        label: 'experiments a typical ML team runs per quarter — impossible to manage without tracking tooling',
      },
    ],
    order: 1,
    keyConcepts: [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'inline-check',
        question: 'Why is bit-for-bit reproducibility often impractical for production ML training?',
        options: [
          'Modern GPUs do not support deterministic operations',
          'Disabling GPU optimizations for determinism can slow training by 20-30%',
          'Python random number generators cannot be seeded',
          'Data shuffling cannot be controlled',
        ],
        correctIndex: 1,
        explanation: 'While bit-for-bit reproducibility is achievable, it requires disabling GPU optimizations (like cuDNN auto-tuning and non-deterministic atomics) that sacrifice determinism for speed. This can slow training by 20-30%, which is costly at scale. Statistical reproducibility (results within a small variance) is usually sufficient for production.',
      },
    ],
    order: 2,
    keyConcepts: [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'inline-check',
        question: 'In a model promotion pipeline, what should happen at the "canary" stage?',
        options: [
          'The model is fully deployed and replaces the old version',
          'A small percentage of live traffic is routed to the new model for evaluation',
          'The model is trained on production data for the first time',
          'Automated unit tests run against the model code',
        ],
        correctIndex: 1,
        explanation: 'The canary stage routes a small percentage (typically 1-10%) of live production traffic to the new model. This allows validation against real-world data and detection of issues that automated tests might miss, such as latency regressions or edge cases. If SLOs are met during the canary period, the model is promoted to full production.',
      },
      {
        type: 'playground',
        title: 'Canary Deployment Risk Calculator',
        description: 'Explore how canary traffic percentage and deployment duration affect the risk exposure during model rollout.',
        parameters: [
          { name: 'canaryPercent', label: 'Canary traffic %', min: 1, max: 50, step: 1, default: 5, unit: '%' },
          { name: 'durationHours', label: 'Canary duration', min: 1, max: 168, step: 1, default: 24, unit: 'hours' },
          { name: 'requestsPerHour', label: 'Requests/hour (K)', min: 1, max: 1000, step: 1, default: 100, unit: 'K' },
        ],
        computeFn: 'canaryDeploymentRisk',
        chartType: 'gauge',
      },
    ],
    order: 3,
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'playground',
        title: 'Grid vs Random Search Efficiency',
        description: 'Compare how many configurations grid search and random search can explore given a fixed trial budget across multiple hyperparameter dimensions.',
        parameters: [
          { name: 'dimensions', label: 'Hyperparameters', min: 1, max: 8, step: 1, default: 3, unit: 'dims' },
          { name: 'budget', label: 'Trial budget', min: 10, max: 500, step: 10, default: 100, unit: 'trials' },
        ],
        computeFn: 'gridVsRandomSearch',
        chartType: 'bar',
      },
      {
        type: 'inline-check',
        question: 'Why is random search generally more efficient than grid search for hyperparameter tuning?',
        options: [
          'Random search uses less memory',
          'Random search covers more of the important dimensions because it does not waste trials on unimportant ones',
          'Random search always finds the global optimum',
          'Random search runs faster per trial',
        ],
        correctIndex: 1,
        explanation: 'Grid search wastes trials exhaustively covering unimportant dimensions. If only 2 out of 5 hyperparameters actually matter (common in practice), grid search evaluates many redundant combinations of the 3 unimportant ones. Random search distributes trials more evenly across the important dimensions, finding good configurations faster (Bergstra & Bengio, 2012).',
      },
    ],
    order: 4,
    keyConcepts: [
