...
export const sections: ChapterSection[] = [
  {
    ...
    blocks: [
      ...
      {
        type: 'aha',
        highlight: 'The most dangerous pipeline failures are silent — the pipeline runs successfully, but the data is wrong.',
        explanation: 'Unlike a software crash that immediately alerts you, a data pipeline can produce output that looks correct but has subtle corruption: a feature column filled with defaults, a join that drops 30% of records, or a timezone conversion that shifts all timestamps. The model trains happily on this bad data and produces worse predictions with no error message anywhere.',
        analogy: 'Imagine a water treatment plant where the quality sensors fail silently. Clean-looking water flows through the pipes, tests show "OK," but the fluoride levels are 10x too high. Nobody notices until people get sick. Data pipelines need their own quality sensors — validation checks at every stage.',
      },
    ],
    order: 0,
    keyConcepts: [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'playground',
        title: 'Labeling Cost Calculator',
        description: 'Estimate the cost difference between random labeling and active learning for your dataset.',
        parameters: [
          { name: 'datasetSize', label: 'Unlabeled samples', min: 1000, max: 1000000, step: 1000, default: 100000, unit: 'samples' },
          { name: 'costPerLabel', label: 'Cost per label', min: 0.01, max: 10, step: 0.01, default: 0.50, unit: '$' },
          { name: 'activeLearningRatio', label: 'Active learning efficiency', min: 2, max: 10, step: 1, default: 4, unit: 'x' },
        ],
        computeFn: 'labelingCostCalculator',
        chartType: 'bar',
      },
      {
        type: 'stat',
        value: 4,
        suffix: 'x',
        label: 'fewer labels needed with active learning vs. random sampling to reach the same model accuracy',
      },
      {
        type: 'inline-check',
        question: 'How does active learning select which examples to label next?',
        options: [
          'It randomly selects examples from the dataset',
          'It picks the examples where the current model is most uncertain',
          'It selects the largest examples in the dataset',
          'It chooses examples that are most similar to already-labeled data',
        ],
        correctIndex: 1,
        explanation: 'Active learning uses the current model\'s uncertainty to select the most informative examples for human annotation. By labeling examples where the model is least confident, each new label provides maximum information for improving the model, making active learning much more sample-efficient than random selection.',
      },
    ],
    order: 1,
    keyConcepts: [
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'inline-check',
        question: 'Why is data lineage especially important for companies operating in the EU?',
        options: [
          'EU regulations require all data to be stored in European data centers',
          'GDPR requires the ability to explain how automated decisions are made, which needs tracing from predictions back to training data',
          'European data formats are incompatible with US storage systems',
          'EU law prohibits using data augmentation',
        ],
        correctIndex: 1,
        explanation: 'GDPR and similar regulations require organizations to explain how automated decisions are made. Data lineage provides the audit trail needed to trace from a model prediction back through the training pipeline to the original source data. Without lineage, demonstrating compliance with explainability requirements is extremely difficult.',
      },
    ],
    order: 2,
    keyConcepts: [
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        value: 10,
        suffix: '%',
        label: 'label error rate found in production crowd-sourced data — high enough to meaningfully degrade model quality',
      },
      {
        type: 'inline-check',
        question: 'What is a data contract in the context of ML systems?',
        options: [
          'A legal agreement about data ownership and intellectual property',
          'A formal specification of schema, quality constraints, and SLAs between data producers and consumers',
          'A database transaction that ensures ACID properties',
          'A pricing agreement with a data labeling vendor',
        ],
        correctIndex: 1,
        explanation: 'A data contract is a formal agreement between data producers (upstream teams) and consumers (ML pipeline) that specifies the expected schema, quality constraints, freshness requirements, and SLAs for a data source. Data contracts prevent upstream changes from silently breaking downstream ML pipelines.',
      },
    ],
    order: 4,
    keyConcepts: [
